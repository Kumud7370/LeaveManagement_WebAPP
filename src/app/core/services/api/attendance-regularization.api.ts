...
export class AttendanceRegularizationService {
  ...
  requestRegularization(dto: RegularizationRequestDto): Observable<ApiResponse<RegularizationResponseDto>> {
    return this.apiClient.post<ApiResponse<RegularizationResponseDto>>(
      `${this.endpoint}/request`, dto);
  }
  ...
  approveRegularization(
    ...
    return this.apiClient.patch<ApiResponse<RegularizationResponseDto>>(
      `${this.endpoint}/${id}/approve`, dto);
  }

  getById(id: string): Observable<ApiResponse<RegularizationResponseDto>> {
    return this.apiClient.get<ApiResponse<RegularizationResponseDto>>(
      `${this.endpoint}/${id}`);
  }
  ...
  getFiltered(
    filter: RegularizationFilterDto
  ): Observable<ApiResponse<PaginatedResponse<RegularizationResponseDto>>> {
    return this.apiClient.post<ApiResponse<PaginatedResponse<RegularizationResponseDto>>>(
  ...
  getPendingCount(employeeId: string): Observable<ApiResponse<number>> {
    return this.apiClient.get<ApiResponse<number>>(
      `${this.endpoint}/pending-count/${employeeId}`);
  }
  ...
  /**
   * ✅ Calls DELETE /api/AttendanceRegularization/{id}
   * Soft-deletes the record on the backend (sets IsDeleted = true).
   */
  deleteRegularization(id: string): Observable<ApiResponse<boolean>> {
