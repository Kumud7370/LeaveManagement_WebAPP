...
export class ApiClientService {
  ...
  /**
   * Primary login — stores all session data on success.
   */
  public loginUser(credentials: { username: string; password: string; deviceId: string }): Observable<any> {
    return this.post('Auth/login', credentials).pipe(
      map((response: any) => {
   ...
   */
  public changePassword(data: { currentPassword: string; newPassword: string; confirmPassword: string }): Observable<any> {
    return this.post('Auth/change-password', data);
  }
   ...
   * Role must be: "Tehsildar", "NayabTehsildar", or "Employee".
   * For Employee role, employeeId (existing employee record) is required.
   */
  public adminCreateUser(data: {
    username: string;
    password: string;
    firstName: string;
    lastName: string;
    ...
    return this.post('admin-management/users', data);
  }

  /** Admin updates a user's profile (name / email). */
  public adminUpdateUser(userId: string, data: {
    firstName?: string;
    lastName?: string;
    email?: string;
  }): Observable<any> {
    return this.put(`admin-management/users/${userId}`, data);
  }

  /** Admin activates or deactivates a user account. */
  public adminSetUserStatus(userId: string, isActive: boolean): Observable<any> {
    return this.patch(`admin-management/users/${userId}/status`, isActive);
  }

  /** Admin deletes a user (soft delete). */
  public adminDeleteUser(userId: string): Observable<any> {
    return this.delete(`admin-management/users/${userId}`);
  }

  /** Admin fetches all user accounts. */
  public adminGetAllUsers(): Observable<any> {
    return this.get('admin-management/users');
  }

  /** Admin fetches a single user by ID. */
  public adminGetUserById(userId: string): Observable<any> {
    return this.get(`admin-management/users/${userId}`);
  }

  /**
   * Admin directly resets another user's password.
   * Body: { newPassword: string }
   */
  public adminResetUserPassword(userId: string, newPassword: string): Observable<any> {
    return this.post(`admin-management/users/${userId}/reset-password`, { newPassword });
  }
