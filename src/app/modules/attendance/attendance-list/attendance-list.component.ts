...
export class AttendanceListComponent implements OnInit, OnDestroy {
  ...
  columnDefs: ColDef[] = [
      ...
      cellRenderer: (params: any) => {
        ...
        const approveBtn = !isApproved
          ? `<button class="btn-icon btn-approve" data-action="approve" title="Approve">
               <i class="bi bi-check2" style="pointer-events:none"></i>
             </button>`
          : '';
    ...
    {
      headerName: 'Emp Code', field: 'employeeCode', width: 110, minWidth: 90,
      cellRenderer: (params: any) => {
    ...
    {
      headerName: 'Employee', field: 'employeeName', flex: 1, minWidth: 150,
      cellRenderer: (params: any) => {
        const val = params.value || '';
        const search = params.context?.componentParent?.searchEmployeeId || '';
        const hl = params.context?.componentParent?.highlightText(val, search) || val;
    ...
    {
      headerName: 'Date', field: 'attendanceDate', width: 140, minWidth: 120,
    ...
    {
      headerName: 'Check In', field: 'checkInTime', width: 110, minWidth: 90,
      valueFormatter: (params: any) =>
        params.value ? new Date(params.value).toLocaleTimeString('en-IN', { hour: '2-digit', minute: '2-digit', hour12: true }) : '—',
      cellStyle: { color: '#374151' }
    },
    {
      headerName: 'Check Out', field: 'checkOutTime', width: 110, minWidth: 90,
      valueFormatter: (params: any) =>
        params.value ? new Date(params.value).toLocaleTimeString('en-IN', { hour: '2-digit', minute: '2-digit', hour12: true }) : '—',
      cellStyle: { color: '#374151' }
    },
    {
      headerName: 'Hours', field: 'workingHours', width: 90, minWidth: 80,
      valueFormatter: (params: any) => params.value != null ? `${(+params.value).toFixed(1)}h` : '—',
    ...
    },
    {
      ...
      cellRenderer: (params: any) => {
        ...
        return `<span style="display:inline-flex;align-items:center;padding:3px 10px;border-radius:9999px;
                             font-size:12px;font-weight:600;background:${s.bg};color:${s.color};border:1px solid ${s.border};">
                  ${label}</span>`;
      }
    },
    {
      headerName: 'Late', field: 'isLate', width: 90, minWidth: 75,
    ...
    {
      headerName: 'Method', field: 'checkInMethodName', width: 120, minWidth: 100,
  ...
  loadAttendance(): void {
    ...
    this.attendanceService.getFilteredAttendance(filter).subscribe({
      ...
      error: (error: any) => {
        this.isLoadingData = false;
        if (error.status !== 401) Swal.fire({ icon: 'error', title: 'Error', text: 'Failed to load attendance records.' });
      }
    });
  }
  ...
  loadStats(): void {
    ...
    this.attendanceService.getStatistics(from.toISOString().substring(0, 10), today.toISOString().substring(0, 10)).subscribe({
      next: (r: any) => {
        if (r.success) {
  ...
  onSearchChange(): void {
    ...
    this.searchDebounceTimer = setTimeout(() => { this.currentPage = 1; this.loadAttendance(); this.updateActiveFilters(); }, 350);
  }
  ...
  updateActiveFilters(): void {
    ...
    if (colCount > 0) extra['Column filters'] = `${colCount} active`;
    const filters = getActiveFiltersSummary(this.searchEmployeeId, undefined, extra);
  ...
  onPageSizeChanged(newSize: number): void { this.pageSize = newSize; this.currentPage = 1; this.loadAttendance(); }

  goToPage(page: number | string): void {
    const p = typeof page === 'string' ? parseInt(page, 10) : page;
    if (isNaN(p) || p < 1 || p > this.totalPages || p === this.currentPage) return;
    this.currentPage = p; this.loadAttendance();
  }
  ...
  viewDetails(record: AttendanceResponseDto): void {
    this.selectedAttendanceId = record.id; this.showDetails = true; this.showForm = false;
  }
  openEditForm(record: AttendanceResponseDto): void {
    this.editingRecord = record; this.showForm = true; this.showDetails = false;
  }
  openCreateForm(): void {
    this.editingRecord = null; this.showForm = true; this.showDetails = false;
  }
  onSidebarClose(): void {
    this.showDetails = false; this.showForm = false;
    this.selectedAttendanceId = null; this.editingRecord = null;
    this.sizeColumns();
  }
  onOverlayClick(event: Event): void { if (event.target === event.currentTarget) this.onSidebarClose(); }
  onRecordSaved():   void { this.onSidebarClose(); this.loadAttendance(); this.loadStats(); }
  onRecordUpdated(): void { this.loadAttendance(); this.loadStats(); }
  ...
  async approveRecord(record: AttendanceResponseDto): Promise<void> {
    const res = await Swal.fire({
      ...
      confirmButtonColor: '#10b981', cancelButtonColor: '#6b7280', confirmButtonText: 'Approve'
    });
    ...
    this.attendanceService.approveAttendance(record.id).subscribe({
      next: (r: any) => {
        if (r.success) {
          ...
          this.loadAttendance(); this.loadStats();
    ...
    const res = await Swal.fire({
      ...
      html: `<div style="text-align:left;padding:10px;">
               ...
               <p style="color:#ef4444;margin-top:15px;padding:10px;background:#fee2e2;border-radius:6px;">
                 <strong>Warning:</strong> This action cannot be undone.</p></div>`,
    ...
    this.attendanceService.deleteAttendance(record.id).subscribe({
      next: (r: any) => {
        if (r.success) {
          ...
          this.totalPages = Math.ceil(this.totalItems / this.pageSize);
          if (this.rowData.length === 0 && this.currentPage > 1) { this.currentPage--; this.loadAttendance(); }
