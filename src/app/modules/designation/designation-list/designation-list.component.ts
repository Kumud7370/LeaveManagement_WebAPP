...
export class DesignationListComponent implements OnInit, OnDestroy {
  ...
  // ─── Filters ──────────────────────────────────────────────────────────────
  searchTerm           = '';
  selectedDepartmentId = '';          // '' = All departments
  ...
  /** Cast helper for strict-mode templates */
  ...
  initializeForm(): void {
    this.designationForm = this.fb.group({
      ...
      level:           [1,  [Validators.required, Validators.min(1), Validators.max(100)]],
      isActive:        [true],
      departmentId:    [''],   // optional department assignment on the designation itself
    });
  }
    ...
    this.columnDefs = [
      ...
      },
      {
        ...
        cellRenderer: (params: ICellRendererParams) => {
          ...
          return `<span style="display:inline-flex;align-items:center;justify-content:center;
                               padding:0.2rem 0.65rem;border-radius:999px;font-size:0.78rem;
                               font-weight:600;background:#e0e7ff;color:#4338ca;">${count} Employee(s)</span>`;
        },
      },
      {
        headerName: 'Description',
  ...
  loadDesignations(): void {
    ...
    // When a department is selected use the by-department endpoint,
    // otherwise use the standard paginated filter endpoint.
    if (this.selectedDepartmentId) {
      this.designationService.getDesignationsByDepartment(this.selectedDepartmentId).subscribe({
        next: (r: any) => {
          // Returns ApiResponse<DesignationResponseDto[]>  →  r.data is the array
          const arr: DesignationResponseDto[] = r?.data ?? [];

          // Client-side search filter when a department is active
  ...
  openCreateModal(): void {
    ...
    this.designationForm.reset({
      designationCode: '', designationName: '', description: '',
      level: 1, isActive: true, departmentId: ''
    });
  }
