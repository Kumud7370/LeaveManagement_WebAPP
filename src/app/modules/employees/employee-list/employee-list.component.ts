...
export class EmployeeListComponent implements OnInit, OnDestroy {
  ...
  // ─── Departments for reassign modal ───────────────────────────────────────
  ...
  /** Cast EventTarget to HTMLSelectElement for strict-mode templates */
  ...
  onDepartmentChange(departmentId: string): void {
    ...
    this.designationService.getDesignationsByDepartment(departmentId).subscribe({
      next: (r: any) => {
        // getDesignationsByDepartment returns ApiResponse<DesignationResponseDto[]>
        // After mapId(), each item has `designationId` guaranteed
        const raw = r?.data ?? r;
        const arr: any[] = Array.isArray(raw)
          ? raw
          : Array.isArray(raw?.items) ? raw.items : [];
        this.designations = arr.map((d: any) => ({
  ...
  submitReassign(): void {
    ...
    this.employeeService.reassignEmployee(this.reassignEmployee.id, dto).subscribe({
      next: (r: any) => {
        if (r?.success !== false) {
          Swal.fire({
            icon: 'success', title: 'Reassigned!',
            text: 'Employee reassigned successfully.',
      ...
      error: (e: any) => Swal.fire('Error', e.error?.message || 'Failed to reassign', 'error')
    });
  }
  ...
  openHistoryModal(employee: EmployeeResponseDto): void {
    ...
    this.employeeService.getAssignmentHistory(employee.id).subscribe({
      next: (data: AssignmentHistoryResponseDto[]) => {
